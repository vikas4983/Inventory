...
@section('content')
    ...
    <a href="{{ route('statuses.create') }}" title="{{ __('titles.add_status') }}"
        class="btn btn-secondary">{{ __('buttons.add_new_status') }}</a>
                ...
                <tr>
                    ...
                    <td>
                        <div style="display: flex; flex-wrap: nowrap; gap: 6px; margin-bottom: 0.5rem;">
                            <x-buttons.edit-button :objectData="$status" :url="route('statuses.edit', $status->id)" :title="'status'" :method="'GET'"
                                :modalSize="__('labels.status_edit_modal')" />
